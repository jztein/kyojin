...
HorizontalSliderPlayer::HorizontalSliderPlayer()
{
	...
	vel = float2(0, 0);
	accel.y = 1000;
	weapons_level = HoriWeaponsLevel::horisingle;
}
...
void HorizontalSliderPlayer::Shoot()
{
	...
	{
	case HoriWeaponsLevel::horisingle:
		...
		break;
	case HoriWeaponsLevel::horisingle_medium:
		data.SetPos(pos);
		data.vel = float2(1000.0f* cos(rot), -1000.0f*sin(rot));
		...
		bullets.push_back(data);
		break;
	case HoriWeaponsLevel::horitriple:
		...
		break;
	case HoriWeaponsLevel::horitriple_medium:
...
{
	if (weapons_level < 4)
		weapons_level = HoriWeaponsLevel((int)weapons_level + 1);
}
