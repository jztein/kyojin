...
void RocketFire::Draw(BasicSprites::SpriteBatch^ m_spriteBatch)
{
	...
	{ 
		...
		m_spriteBatch->Draw(
			...
			posoffset,
			PositionUnits::DIPs,
			float2(70.0f, 30.0f)*(-particle->x-101.f) / 50.f,
			SizeUnits::DIPs,
...
void RocketFire::Draw(BasicSprites::SpriteBatch^ m_spriteBatch, float factor)
{
	for (auto particle = particles.begin(); particle != particles.end(); particle++)
	{
		float2 posoffset = float2(pos.x + particle->x *cos(rot),
		...
		m_spriteBatch->Draw(
			...
			posoffset,
			PositionUnits::DIPs,
			float2(50.0f, 20.0f)*factor*(-particle->x - 101.f) / 50.f,
			SizeUnits::DIPs,
			float4(.6f, .4f, .4f, 1.0f),
			0.0f,
			BlendMode::Additive
